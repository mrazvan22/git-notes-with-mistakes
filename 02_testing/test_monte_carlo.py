...
import numpy as np
from monte_carlo import move_particle, monte_carlo
...
def test_monte_carlo_zero_particles():
  ...
  with assert_raises(ValueError) as exception:    monte_carlo(density, T, iterations)

def test_monte_carlo_one_particle():
  density = [0,0,1,0]
  T = 2.0
  iterations = 20
  densities = monte_carlo(density, T, iterations)
